"""
Utilities for converting SmartSPIM JSON metadata to BigStitcher XML format.
"""
...
def extract_tile_channel_numbers(json_dict: dict):
    """Extracts the channel numbers from the json_dict"""
    tile_channel_list = []
...
def get_tile_number_lookup(json_dict: dict):
    """Get tile number lookup from json dict"""
    # TODO rewrite this to make only a minimal subset of tiles,
    # so that channels that share a tile are not duplicated
...
def extract_tile_names(json_dict: dict) -> list[str]:
    """Extracts the tile names from the json_dict"""
...
def extract_tile_names_unaltered(json_dict: dict) -> list[str]:
    """Extracts the tile names from the json_dict without altering them"""
    tile_names: list[str] = []
    for row in json_dict:
        # Ex: TILE_XXXX_ch_0.zarr
...
def extract_tile_sizes(json_dict: dict) -> list[list[int]]:
    """Extracts the tile sizes from the json_dict"""
...
def extract_tile_resolution(json_dict: dict, microns=False) -> list[float]:
    """Extracts the tile resolution from the json_dict"""
    try:
...
def extract_tile_translations(json_dict: dict) -> list[list[float]]:
    """Extracts the tile translations from the json_dict"""
...
def add_sequence_description(
    ...
    """Adds the SequenceDescription element to the XML tree"""
    ...
    def add_image_loader(
        ...
        """Adds the ImageLoader element to the SequenceDescription"""
    ...
    def add_view_setups(
        ...
        """Adds the ViewSetups element to the SequenceDescription"""

        def add_attributes(
            view_setups: ET.Element,
            unique_channel_list: list[int],
        ) -> None:
            """Adds the Attributes element to the ViewSetups"""
            # Add attributes
    ...
    def add_time_points(seq_desc: ET.Element) -> None:
        """Adds the TimePoints element to the SequenceDescription"""
...
def add_view_registrations(parent: ET.Element, translations: list[list[float]]) -> None:
    """Adds the ViewRegistrations element to the XML tree"""
...
def parse_json(json_path: str, s3_data_path: str, microns=False) -> ET.ElementTree:
    """Parses the JSON file and constructs the XML tree for BigStitcher"""
...
def convert_json_to_xml(json_loc, s3_data_path, xml_name=None):
    """Converts SmartSPIM JSON metadata to BigStitcher XML format"""
